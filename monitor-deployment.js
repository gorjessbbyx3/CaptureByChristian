...
async function checkHealth(baseUrl) {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      log(`✅ ${endpoint.name}: ${response.statusCode} (${responseTime}ms)`, colors.green);
...
async function checkDeployment(baseUrl) {
  try {
    ...
    log(`🔍 Checking deployment status for commit ${DEPLOYMENT_COMMIT}...`, colors.blue);
    ...
    if (response.statusCode === 200) {
      ...
      if (response.body.includes('path-to-regexp') || response.body.includes('Missing parameter')) {
        log(`⚠️  Path-to-regexp error still present in response`, colors.yellow);
        return false;
      }
      
      // Check if all health endpoints are working
      const healthSuccess = Object.values(healthResults).every(result => result.success);
      
      if (healthSuccess) {
        log(`🎉 DEPLOYMENT SUCCESSFUL! All endpoints healthy.`, colors.bold + colors.green);
      ...
      if (response.body.includes('path-to-regexp') || response.body.includes('Missing parameter')) {
        ...
        log(`📝 This suggests the fix in commit ${DEPLOYMENT_COMMIT} may not be working`, colors.yellow);
      }
      
      return false;
      
    } else {
      log(`🔄 Service returning ${response.statusCode}, possibly still deploying...`, colors.yellow);
      return false;
    }
...
async function monitorDeployment(serviceUrl) {
  log(`🚀 Starting deployment monitor for commit ${DEPLOYMENT_COMMIT}`, colors.bold + colors.blue);
  ...
  while (retryCount < MAX_RETRIES) {
    ...
    if (success) {
      log(``, colors.reset);
      log(`🎊 MONITORING COMPLETE - DEPLOYMENT SUCCESSFUL!`, colors.bold + colors.green);
      process.exit(0);
    }
    
    if (retryCount < MAX_RETRIES) {
      log(`⏳ Waiting ${CHECK_INTERVAL/1000} seconds before next check...`, colors.cyan);
      log(``, colors.reset);
  ...
  log(``, colors.reset);
  log(`⏰ Maximum monitoring time exceeded (${MAX_RETRIES * CHECK_INTERVAL/1000/60} minutes)`, colors.red);
  log(`🚨 Deployment may have failed or is taking longer than expected`, colors.red);
  process.exit(1);
}
...
if (!serviceUrl) {
  console.log(`${colors.red}❌ Please provide your Render service URL${colors.reset}`);
  console.log(`${colors.cyan}Usage: node monitor-deployment.js <render-service-url>${colors.reset}`);
  console.log(`${colors.cyan}Example: node monitor-deployment.js https://your-service.onrender.com${colors.reset}`);
  process.exit(1);
}
...
try {
  ...
  console.log(`${colors.red}❌ Invalid URL format: ${serviceUrl}${colors.reset}`);
  process.exit(1);
}
